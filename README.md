...
- **Critical**: When downloading the Ghost repository, you must use the `npm install` method to set up dependencies correctly.
...
1. Check if `@ghost.build/cli` is already installed globally via npm
   - If missing: runs `npm install -g @ghost.build/cli`
2. Check if `pi-mcp-adapter` is already installed via Pi
   - If missing: runs `pi install npm:pi-mcp-adapter`
3. Works cross-platform (Windows, macOS, Linux) with no extra configuration required
